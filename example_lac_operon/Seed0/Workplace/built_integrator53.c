...
/**************degradation rates*****************/
	 	 rate=0.010074288836874223*s[0];
	 	 increment=rate;
	 	 ds[0]-=increment;
	 	 rate=0.9616871792872654*s[1];
	 	 increment=rate;
	 	 ds[1]-=increment;
...
/**************Transcription rates*****************/
	 ...
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.443640*HillA(history[2][memory][ncell],0.877045,4.870225),0.000000);
	 	 increment=rate;
	 	 ds[2]+=increment;
	}
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.656566,0.000000);
	 	 increment=rate;
	 	 ds[3]+=increment;
	}
