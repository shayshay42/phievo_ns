...
static int tracklig[] = {};
static int trackdiff[] = {2, 3, 4};
static double diff_constant[] = {0.5584309375598064, 0.12448035388436374, 0.26576298054259817};
...
/**************degradation rates*****************/
	 	 rate=0.5120721446837272*s[0];
	 	 increment=rate;
	 	 ds[0]-=increment;
	 	 rate=0.5651374365531077*s[1];
	 	 increment=rate;
	 	 ds[1]-=increment;
	 	 rate=0.5846656260268385*s[2];
	 	 increment=rate;
	 	 ds[2]-=increment;
	 	 rate=0.2956302325408833*s[3];
	 	 increment=rate;
	 	 ds[3]-=increment;
	 	 rate=0.3440226300815977*s[4];
	 	 increment=rate;
	 	 ds[4]-=increment;
	 	 rate=0.6722691486390298*s[5];
	 	 increment=rate;
	 	 ds[5]-=increment;
...
/**************Transcription rates*****************/
	 ...
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.646489*MAX(HillA(history[2][memory][ncell],0.751126,2.263021),HillA(history[4][memory][ncell],0.134219,1.286393)),0.000000);
	 	 increment=rate;
	 	 ds[2]+=increment;
	}
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.692058,0.000000)*HillR(history[4][memory][ncell],0.599111,1.758863)*HillR(history[3][memory][ncell],0.566811,4.479687);
	 	 increment=rate;
	 	 ds[3]+=increment;
	}
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.067561,0.000000);
	 	 increment=rate;
	 	 ds[4]+=increment;
	}

/**************Protein protein interactions*****************/
	 	 rate=0.404633 * s[4] * s[0];
	 	 increment=rate;
	 	 ds[4]-=increment;
	 	 ds[0]-=increment;
	 	 ds[5]+=increment;
