...
/**************Transcription rates*****************/
	 ...
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.646489*MAX(HillA(history[2][memory][ncell],0.072135,4.505847),HillA(history[4][memory][ncell],0.641633,1.156661)),0.000000);
	 	 increment=rate;
	 	 ds[2]+=increment;
	}
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.692058,0.000000)*HillR(history[4][memory][ncell],0.599111,1.758863);
	 	 increment=rate;
	 	 ds[3]+=increment;
	}
	 memory=step-0;
